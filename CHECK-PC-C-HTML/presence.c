...
void write_lines_of_html(student *sdt, int nbr_lines)
{
	...
	char today_date[15] = "";
	get_date(today_date);
	sprintf(html_path, "EXPORT_HTML/%s.html", today_date); 

	fd = open_file(html_path,"w+");
	int i=0; 
	...
	"<!DOCTYPE html>\n"
		"<html>\n"
			"<head>\n"
				"<title>PRESENCE L1</title>\n"
    			"<link rel=\"stylesheet\" href=\"assets/css/style.css\" />\n"
  		"</head>\n"
		"<body>\n"
    		"<div class=\"container\">\n"
			"<div class=\"menu hide-left\">\n"
				"<h1>Presence L1</h1>\n"
				"<div class=\"widgets\">\n"
					"<div>\n"
						"<img src=\"assets/images/logo_mit.png\" alt=\"Logo MIT\" />\n"
					"</div>\n"
					"<div>\n"
						"<div class=\"button-box\">\n"
						"<span>List with filter</span>\n"
						"<button class=\"button-filter active\">ALL</button>\n"
						"<button class=\"button-filter\">PRESENT</button>\n"
						"<button class=\"button-filter\">ABSENT</button>\n"
						"<button class=\"button-filter\">YES</button>\n"
						"<button class=\"button-filter\">PERSO</button>\n"
						"<button class=\"button-filter\">NO</button>\n"
						"</div>\n"
					"</div>\n"
				"</div>\n"
			"</div>\n"
			"<div class=\"container-tab hide-right\">\n"
				"<h1>Presence 26/07/2024</h1>\n"
				"<table>\n"
				"<thead>\n"
					"<tr>\n"
					"<th>Numéro</th>\n"
	...
	{
		char pc_state[10] = "";
		if (sdt[i].get_machine == NO) strcpy(pc_state, "NO");
		else if (sdt[i].get_machine == YES) strcpy(pc_state, "YES");
		else if (sdt[i].get_machine == PERS) strcpy(pc_state, "PERSO");
		else strcpy(pc_state, "ABSENT");
		...
		"<tr>\n"
			...
			"<td>%s</td>\n"
			"<td>%s</td>\n"
			"<td class=\"get-machine\">%s</td>\n"
		"</tr>\n",
		i+1, sdt[i].time, sdt[i].name, pc_state); 
...
void remove_one_character(char *array, int n)
{
	size_t i;
	if (!array)	exit(EXIT_FAILURE);
	for (i=n; i<strlen(array); i++) array[i] = array[i+1];
	array[i] = '\0';
}